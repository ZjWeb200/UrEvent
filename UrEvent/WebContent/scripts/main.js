(function() {
  ...
  function login() {
    ...
    // hash the password for privacy issue. in this case, database only save the hash instead of the password itself.
    password = md5(username + md5(password));     

    // The request parameters
    var url = './login';
    // turn js object to a JSON format
    var req = JSON.stringify({
      user_id : username,
      password : password,
    });
  ...
  function register() {
    ...
    if (username.match(/^[a-zA-Z0-9_]+$/) === null) {
    	showRegisterResult('Invalid username. (Username should be in lowercase letter, uppercase letter, digit, or underscore.)');
    	return
    }
    ...
    ajax('POST', url, req,
      ...
      function(res) {
        ...
        // successfully logged in
        if (result.status === 'OK') {
        	showRegisterResult('Succesfully signed up!');
        } else {
  ...
  function ajax(method, url, data, successCallback, errorCallback) {
    ...
    // ready state is 4. i.e. we get a response back.
    xhr.onload = function() {
  ...
  function addItem(itemList, item) {
    ...
    date.innerHTML = 'Event date: ' + item.local_date;
